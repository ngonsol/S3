...
export default {
    ...
    metadataValidateAuthorization(params, cb) {
        const { authInfo, bucketName, objectKey, requestType, log, query } =
            params;
        const canonicalID = authInfo.getCanonicalID();
     ...
     * @param {object} query - query of the request
     * @return {undefined}
     */
    deleteObject(bucketName, objectMD, responseMDHeaders, objectKey, log, cb,
        ...
        // delete the data in the cases of non-versioning or version specific
        if (objectMD['x-amz-version-id'] === 'null' || query.versionId) {
            // non-versioned buckets
            log.trace('deleteObject: deleting the object');
            ...
            async.eachLimit(objectMD.location, 5, (loc, ok) => {
                ...
                metadata.deleteObjectMD(bucketName, objectKey, log, cb, query);
            });
        } else {
            // versioning
            ...
            return metadata.deleteObjectMD(bucketName, objectKey, log,
                (err, data) => {
                    if (data) {
                        ...
                        _data['x-amz-version-id'] = _data.versionId ?
                            _data.versionId : _data['x-amz-version-id'];
                        return cb(err, _data);
                    }
                    return cb(err, data);
                    // no deletion of data in versioning
                }, query);
    ...
    validateHeaders(objectMD, metadataCheckParams, cb) {
        ...
        if (objectMD['x-amz-version-id']) {
            responseMetaHeaders['x-amz-version-id'] =
                objectMD['x-amz-version-id'];
        }
