...
const metadata = {
    ...
    putObjectMD: (bucketName, objName, objVal, log, cb) => {
        ...
        client.putObject(bucketName, objName, objVal, log, (err, data) => {
            if (err) {
                log.warn('error from metadata', { implName, error: err });
                return cb(err);
            }
    ...
    getBucketAndObjectMD: (bucketName, objName, log, cb, query) => {
    ...
    getObjectMD: (bucketName, objName, log, cb, query) => {
    ...
    deleteObjectMD: (bucketName, objName, log, cb, query) => {
        log.debug('deleting object from metadata');
        client.deleteObject(bucketName, objName, log, (err, data) => {
            if (err) {
                log.warn('error from metadata', { implName, err });
                return cb(err);
            }
